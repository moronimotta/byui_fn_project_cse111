...
class Controller():
    def create_receipt(self, connection, client_name, total, payment_method, payment_date, items):
        Adapter.create_receipt(connection, client_name, total, payment_method, payment_date, items)

    def get_all_receipts(self, connection):
        receipts = Adapter.get_all_receipts(connection)
        return receipts

    def get_receipt_by_uuid(self, connection, uuid):
        receipt = Adapter.get_receipt_by_uuid(connection, uuid)
        return receipt

    def update_receipt(self, connection, uuid, client_name, total, payment_method, payment_date, items):
        Adapter.update_receipt(connection, uuid, client_name, total, payment_method, payment_date, items)

    def delete_receipt(self, connection, uuid):
        Adapter.delete_receipt(connection, uuid)

    def get_receipts_by_client_name(self, connection, client_name):
